...
const registerUser = asyncHandler(async (req, res) => {
    ...
    const { name, emailorUser, password } = req.body
    ...
    const userExists = await User.findOne({ emailorUser })
    if (userExists) {
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // Look up the user by email/username — returns null if no match
    const user = await User.findOne({ emailorUser }) // from database but encrypted
...
const getMe = asyncHandler(async (req, res) => {
    ...
    const { _id, name, emailorUser } = await User.findById(req.user.id)
